...
*/
var isReactNative$1 = (formData) => formData && typeof formData.getParts !== "undefined";
/**
...
var hasOwnProperty$1 = (({ hasOwnProperty }) => (obj, prop) => hasOwnProperty.call(obj, prop))(Object.prototype);
/**
...
function getCleanClone(prototype) {
	if (!prototype) return create$1(null);
	var Constructor = prototype.constructor;
	if (Constructor === Object) return prototype === Object.prototype ? {} : create$1(prototype);
...
function copyArrayLoose(array, state) {
	...
	for (var index = 0, length_2 = array.length; index < length_2; ++index) clone[index] = state.copier(array[index], state);
	return clone;
}
/**
...
function copyObjectLooseLegacy(object, state) {
	...
	for (var key in object) if (hasOwnProperty.call(object, key)) clone[key] = state.copier(object[key], state);
	return clone;
}
...
function createCopier(options) {
	var tagSpecificCopiers = getTagSpecificCopiers(assign({}, DEFAULT_LOOSE_OPTIONS, options));
	...
	function copier(value, state) {
		...
		if (isArray(value)) return array(value, state);
...
function isNode() {
	...
	**/
	return typeof import_browser.default !== "undefined" && !import_browser.default.browser;
}
function isReactNative() {
	return typeof window !== "undefined" && "navigator" in window && "product" in window.navigator && window.navigator.product === "ReactNative";
}
function getNodeVersion() {
	return import_browser.default.versions && import_browser.default.versions.node ? `v${import_browser.default.versions.node}` : import_browser.default.version;
}
function getWindow() {
	return window;
}
...
var import_isString = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
function pThrottle({ limit, interval, strict, onDelay }) {
	...
	function windowedDelay() {
		...
		if (activeCount < limit) activeCount++;
		else {
...
var walkMutate = function walkMutate(input, predicate, mutator, removeUnresolved) {
	...
	if (input && (typeof input === "undefined" ? "undefined" : _typeof(input)) === "object") {
		for (var key in input) if (input.hasOwnProperty(key)) input[key] = walkMutate(input[key], predicate, mutator, removeUnresolved);
...
var import_stringify = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
...
function mixinStringifySafe(data) {
	return Object.defineProperty(data, "stringifySafe", {
		...
		value: function(serializer = null, indent = "") {
			return (0, import_stringify.default)(this, serializer, indent, (key, value) => {
				return { sys: {
					type: "Link",
					linkType: "Entry",
					id: value.sys.id,
...
async function pagedSync(http, query, options) {
	...
	if (!withoutLinkResolution) response.items = resolveResponse(response, {
...
function resolveCircular(data, { resolveLinks, removeUnresolved }) {
	...
	if (resolveLinks) wrappedData.items = resolveResponse(wrappedData, {
		removeUnresolved,
		itemEntryPoints: ["fields"]
	});
	return wrappedData;
...
function createContentfulApi({ http, getGlobalOptions }, options) {
	...
	function maybeEncodeCPAResponse(data, config) {
		var _a;
		if ((_a = config === null || config === void 0 ? void 0 : config.params) === null || _a === void 0 ? void 0 : _a.includeContentSourceMaps) return le(data);
		return data;
	}
	...
	async function makeGetEntries(query, options = {
		...
		validateRemoveUnresolvedParam(query);
		validateSearchParameters(query);
		return internalGetEntries(withAllLocales ? Object.assign(Object.assign({}, query), { locale: "*" }) : query, options);
	}
	...
	async function internalGetEntries(query, options) {
		...
		try {
			const config = createRequestConfig({ query: prepareQuery(query) });
			if (withLocaleBasedPublishing) config.headers = Object.assign(Object.assign({}, config.headers), { "X-Contentful-Locale-Based-Publishing": true });
	...
	async function makeGetAssets(query, options = {
		...
		validateLocaleParam(query, withAllLocales);
		validateSearchParameters(query);
		return internalGetAssets(withAllLocales ? Object.assign(Object.assign({}, query), { locale: "*" }) : query, options);
	}
	...
	async function makeGetAsset(id, query, options = {
		...
		validateLocaleParam(query, withAllLocales);
		validateSearchParameters(query);
		return internalGetAsset(id, withAllLocales ? Object.assign(Object.assign({}, query), { locale: "*" }) : query, options);
	}
...
function create({ http, getGlobalOptions }, options, makeInnerClient) {
	...
	Object.defineProperty(response, "withLocaleBasedPublishing", { get: () => makeInnerClient(Object.assign(Object.assign({}, options), { withLocaleBasedPublishing: true })) });
